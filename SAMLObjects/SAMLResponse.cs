...
public class SAMLResponse {
    ...
    {             
        ...
        List<SAMLUserAttribute> userAttributes = SAMLHelper.GetSAMLUserAttributes(_configuration);
        foreach( var attr in userAttributes ) {
            ...
            if (attrName == "objectidentifier" ) {
                guid = _configuration[$"SAMLTest:SAMLUser:{attrName}"];
            }
            this.Assertion.AttributeStatement.Attribute.Add(new Attribute(attr.Name, _configuration[$"SAMLTest:SAMLUser:{attrName}"]));
        }

        this.Assertion.Conditions.AudienceRestriction.Audience = thisurl;
        this.Assertion.Subject.NameID.Text = guid;
        this.Assertion.AuthnStatement.SessionIndex = Guid.NewGuid().ToString(); ;
    }
