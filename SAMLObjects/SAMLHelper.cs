...
public class SAMLHelper
{
    ...
    public static string FormatSAMLRequest( string authRequest ) {
        return System.Web.HttpUtility.UrlEncode( SAMLHelper.Compress( authRequest ) );
    }
    public static string FormatSAMLRequestLogout( string logoutRequest ) {
        string cdoc = SAMLHelper.Compress( logoutRequest );
        return System.Web.HttpUtility.UrlEncode(cdoc);
    }
    public static string FormatRelayState( string entityID, string Issuer ) {
        return System.Web.HttpUtility.UrlEncode(SAMLHelper.ToBase64(entityID) + "." + SAMLHelper.ToBase64(Issuer));
    }
    public static void ParseRelayState(string relayState, out string entityID, out string Issuer) {
        entityID = null;
        Issuer = null;
        if (!string.IsNullOrWhiteSpace(relayState)) {
            string[] relayStateBits = relayState.Split(".");
            entityID = System.Web.HttpUtility.UrlDecode(SAMLHelper.FromBase64(relayStateBits[0]));
            Issuer = System.Web.HttpUtility.UrlDecode(SAMLHelper.FromBase64(relayStateBits[1]));
        }
    }
    public static string GetSPEntityID( IConfiguration configuration ) {
        return configuration["SAMLTEST:EntityID"].Replace("-", "");        
    }
    public static List<SAMLUserAttribute> GetSAMLUserAttributes( IConfiguration configuration ) {
        return configuration.GetSection("SAMLTEST:SAMLUserAttributes").Get<List<SAMLUserAttribute>>() ?? new();
    }
}
...
/// <summary>
/// For appsettings.json config on user attributes to return
/// </summary>
public class SAMLUserAttribute {
