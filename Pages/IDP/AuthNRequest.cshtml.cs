...
public class AuthNRequestModel : PageModel {
    ...
    /// </summary>
    public AuthNRequestModel(IConfiguration configuration, IMemoryCache cache) {
    ...
    { 
        if ( string.IsNullOrWhiteSpace( SAMLRequest ) || string.IsNullOrEmpty(RelayState)) {
            return BadRequest("Expecting a SAML request and a relay state.");
        }
